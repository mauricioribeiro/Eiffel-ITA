...
class i_sim_laminado:
    ...
    def verificarCompressao(self,Area,Kx,Ky,Kz,Lx,Ly,Lz,fy,d,bf,tf,tw,h,hw,Ix,Iy,Cw,J,gamma1,NcSd):
        ...
        Qa=0
        #Calculo de Qs
        if ( bf/(2*tf)<= 0.56*math.sqrt((self.E)/fy)): #NBR8800: Tabela F.1,Elementos AL, Grupo 4
            Qs = 1
        else:
             if (bf/(2*tf)<= 1.03*math.sqrt((self.E)/fy)): #NBR8800: Item F.2b
                 Qs = 1.415 - 0.74*(bf/(2*tf))*math.sqrt(fy/(self.E)) #NBR8800: Item F.2b
             else:
                 Qs = 0.69*(self.E)/(fy*(bf/(2*tf))**2) #NBR8800: Item F.2b
        #Calculo de Qa
        if (hw/tw <= 1.49*math.sqrt((self.E)/fy)): #NBR8800: Tabela F.1,Elementos AA, Grupo 2
            Qa = 1
        else:
            bef = 1.92*tw*math.sqrt((self.E)/fy)*(1-0.34*tw/hw*math.sqrt((self.E)/fy)) #NBR8800: Item F.3.2
            if (bef >= hw):
                bef=hw;
            else:
                Aef = Area-(hw-bef)*tw #NBR8800: Item F.3.1
        ...
        Rd2 = Qui*Q*Area*fy/gamma1 #NBR8800: Item 5.3.2    
    ...
    def verificarFletorX(self,Cb,fy,fu,d,bf,tf,tw,h,hw,Lb,ry,Zx,gamma1,Iy,J,Cw,Wx,a,Afn,MSd):
        ...
        L=Lb
        Lambda = hw/tw #NBR8800: Tabela G.1, Estado-limite FLA
        ...
        if Lambda <= 5.7*math.sqrt((self.E)/fy): #NBR8800: Tabela G.1, Estado-limite FLA
            ...
            LambdarFLM = 0.83*math.sqrt((self.E)/(0.7*fy)) #NBR8800: Tabela G.1
            MplFLM = Zx*fy
            MrFLM = 0.7*fy*Wx
            McrFLM = 0.69*(self.E)*Wx/LambdaFLM**2
            ...
            LambdaFLA = hw/tw #NBR8800: Tabela G.1
    ...
    def verificarFletorY(self,Cb,fy,fu,d,bf,tf,tw,h,hw,ry,Zy,gamma1,Iy,J,Cw,Wy,Afn,MSd):
        ...
        if Lambda <= 1.4*math.sqrt((self.E)/fy): #NBR8800: Tabela G.1, Estado-limite FLA
            ...
            MplFLM = Zy*fy
            MrFLM = 0.7*fy*Wy
            McrFLM = 0.69*(self.E)*Wy/LambdaFLM**2
            if (LambdaFLM <= LambdapFLM):
    ...
    def verificarCorteY(self,fy,d,bf,tf,tw,h,hw,gamma1,kv,VSd):
        ...
        Lambda = hw/tw #NBR8800: Item 5.4.3.1.1
